...
object FlatMapsProcessors extends App {
  ...
  //val kstrTotal : KStream[String, List[Double]] = kStrFacture.flatMap((k,f) => (k, Array((f.orderLine.numUnits * f.orderLine.unitPrice), (f.orderLine.numUnits), (f.orderLine.unitPrice))))
